...
`knowledge.review_included_studies` bærer det andre overlappet: en systematisk oversikt og primærstudiene den inkluderer er ikke uavhengige kilder. Relasjonen er mange-til-mange og kan derfor ikke ligge på rapportkoblingen. `knowledge.study_units_for_evidence(uuid[])` sier hvor mange uavhengige studier et evidensgrunnlag faktisk hviler på, og slår sammen begge formene for overlapp transitivt — den sletter ingenting, men et grunnlag med tre funn fra to studier står som to enheter og ikke som tre. Både synteseoppgaven og evidensvurderingen bærer grupperingen i materialet sitt *og* er bundet til den gjennom `knowledge.study_unit_digest(uuid[])`: registreres et overlapp etter at oppgaven ble hentet ut, avvises et svar avgitt på den gamle strukturen som foreldet.
