...
| Oppgitt tall ikke gjenfunnet **der kilden navngir feltet** | `uncertain`, feltet føres ikke som kontrollert | Tallet kan stå skrevet med bokstaver, i en annen enhet eller i en tabell som ikke er med i representasjonen. Samme sifferrekke et annet sted teller ikke: «90 % ble bedre» oppgir ingen utvalgsstørrelse på 90, og «15 mg» er en dose og ikke et effektestimat. Tallene søkes dessuten bare i de av funnets ordrette utdrag som selv navngir funnets intervensjon — ikke i hele artikkelen, og ikke i et utdrag som beskriver en annen arm. Bindingen er på setningen, ikke på utdraget: et tall teller bare fra en setning som selv navngir armen, og for estimat og konfidensintervall også endepunktet, fordi et effektmål hører til ett endepunkt hos én arm. Begreper matches med ordgrense: «citalopram» er ikke «escitalopram». Oppgir utdraget flere verdier for samme felt — «fluoxetine, N = 44; sertraline, N = 48; paroxetine, N = 47» — kan kontrollen ikke avgjøre hvilken som er radens, og feltet står uavklart framfor bekreftet |
