...
**Rettighetsgrensen er to Routines, og den ligger i kjøremiljøet.** En Claude Code Routine er en full, autonom sesjon: den har skall, miljøvariablene til kjøremiljøet sitt, og de connectorene den ble opprettet med — og den kan bruke ethvert verktøy fra dem, skriveverktøy medregnet, uten godkjenning underveis. En sesjon som både leser en artikkel Antidep ikke kontrollerer *og* har noe å skrive med, har begge deler samtidig, som er nøyaktig det §63 sier at et ledd ikke skal ha. Grensen kan derfor ikke settes i Antideps kode alene: modell-leddet kjøres i et **eget kjøremiljø uten skrivekapable hemmeligheter og uten connectorer**, og registrering og verifikasjon kjøres etterpå, i et annet miljø, av kommandoer med hver sin identitet. Antidep bidrar med et lag under: modell-leddet nekter å kjøre dersom en skrivekapabel legitimasjon står i miljøet til prosessen. Det fanger det vanligste uhellet, men ser verken sesjonen som startet kjøringen eller connectorene den har, og skal ikke leses som hele grensen (`ROUTINE_EXTRACTION.md` §3).
