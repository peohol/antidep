...
- **Synlig tilbaketrekking og rollback.** Begge er nye append-only hendelser. En tilbaketrekking navngir hvilket forseglet innhold som ble tatt ut av visning, og kjører bevisst ingen gate. En rollback peker på en tidligere publisert versjon, kjører hele gaten på nytt, og krever i tillegg at målets gjeldende innhold faktisk har vært vist — ellers ville «tilbake» betydd «til noe nytt». Historikken slettes aldri, og den leses i kjedens egen rekkefølge framfor på klokka.
