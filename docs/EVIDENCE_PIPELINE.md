...
**Rettighetsgrensen ligger i kjøremiljøet, connectorlisten og repo-tilgangen.** En Claude Code Routine er en full, autonom sesjon: den har skall, miljøvariablene til kjøremiljøet sitt, og de connectorene den ble opprettet med — og den kan bruke ethvert verktøy fra dem, skriveverktøy medregnet, uten godkjenning underveis. En sesjon som både leser en artikkel Antidep ikke kontrollerer *og* har noe å skrive med, har begge deler samtidig, som er nøyaktig det §63 sier at et ledd ikke skal ha. Grensen kan derfor ikke settes i Antideps kode alene: modell-leddet kjøres i et **eget kjøremiljø uten skrivekapable hemmeligheter, uten connectorer og uten pushetilgang til repoet**, og registrering og verifikasjon kjøres etterpå, som en egen operasjon med sin egen identitet. Pushetilgangen hører med i grensen fordi en byggarbeidsflyt som kjører kode fra en pull request med et deploy-token i miljøet, gjør en branch til en vei videre. Antidep bidrar med et lag under: modell-leddet nekter å kjøre dersom en skrivekapabel legitimasjon står i miljøet til prosessen. Det fanger det vanligste uhellet, men ser verken sesjonen som startet kjøringen eller connectorene den har, og skal ikke leses som hele grensen (`ROUTINE_EXTRACTION.md` §3).

**To Routine-kjøringer deler ikke filsystem.** Hver kjøring er en ny økt med en fersk klone, og grensesnittet i modell-leddet er lokale, gitignorerte filer. Oppdraget leveres derfor i Routinens egen prompt, og forslaget hentes ut av den økten som laget det; registreringen er en bevisst operasjon et menneske setter i gang, ikke en andre Routine som har filene til den første. Full automatisering av hele kjeden er dermed ikke etablert, og skal ikke beskrives som om den var det (`ROUTINE_EXTRACTION.md` §3.5).

**Overleveringen er utrygg inndata, også etter at forslaget er kontrollert.** Filen har vært innom en økt som leste utrygt eksternt innhold og som har skall; den kan endres etter at kontrollen kjørte. Registreringen tar derfor imot **oppdraget** som en egen, tiltrodd inndata — redaktørens egen fil, som kommer en annen vei enn forslaget — og kontrollerer kildebindingen og hver katalogverdi mot det på nytt før noe skrives. Det er nettopp den kontrollen den ordrette ikke kan gjøre: et utdrag kan stå ordrett i kilden og likevel være ført på feil virkestoff eller et naboendepunkt. Forespørselsavtrykket **rekonstrueres** av det samme grunnlaget der alle delene finnes — forespørselen er en ren funksjon av oppdraget, representasjonen og promptmalen — og et avvik gir ingen rad. Der det ikke lar seg rekonstruere, fører kjøringen at avtrykket forble en erklæring; resten av `generated_by` er erklæringer, og skal ikke beskrives som verifisert proveniens (§65). Nøyaktig ett av «her er oppdraget» og «dette forslaget har ikke noe oppdrag» er påkrevd for hver registrering, og valget er kallerens. Arbeidsformen bærer også produsenten: den oppdragsbaserte flyten og et maskinutkast uten oppdrag er begge en modells, en redaktørs eget arbeid er et menneskes, og forslagets egen `producer` må stemme med den. To av tre arbeidsformer er en modells med vilje — den ene tilstanden som ikke skal kunne oppstå av en endret fil, er at et maskinutkast føres som et menneskes arbeid. Arbeidsformen er **påkrevd** for hver registrering, også for re-ekstraksjonen av eldre forslag: så lenge den var valgfri, var en kjøring som utelot den en åpen vei rundt kontrollen — ellers ville et maskinutkast kunnet føres som en menneskelig ekstraksjon ved at ett ord ble endret i filen, og `extraction_method` er nettopp det som forteller kontrolløren hva hen etterprøver (§46). Avviket avvises før kjøringen åpnes. Sperren leser ikke forslagets egen `generated_by` for å avgjøre om kontrollen trengs: da ville den utrygge filen bestemt om den skulle kontrolleres. Valget føres i kjøringens manifest.
...
#### Påstanden har to halvdeler, og hver halvdel har sitt eget kontrollledd

Løsningen er ikke å la den lokale bekreftelsen dekke den globale statusen, og
ikke å gjøre kontrolløren til fulltekstleser. Den er å skille de to påstandene,
fordi de har hvert sitt kontrollgrunnlag — og gi hver av dem et eget felt i
...
(`workflow.source_wide_absence_fields(uuid)`). Gaten er dermed **strengere** enn
...
`source_wide_absence` er ikke et semantisk kontrollfelt
(`workflow.semantic_check_fields(uuid)` utelater det), og databasen avviser en
menneskelig kontroll som fører det opp
(`evidence_verifications_source_wide_absence_check`). Kontrolløkten stiller
ikke spørsmålet, og et «ja» på det lokale spørsmålet skal derfor aldri kunne
bokføres som et svar på det globale (DATABASE_ARCHITECTURE.md §29).

Et framtidig menneskelig kontrollobjekt for globalt fravær — en kontrollør som
faktisk har lest hele kilden — er mulig, men er da et **eget** objekt med sin
egen dekning og sin egen flate. Det er ikke en oppmyking av regelen over.
...
Den implementerte verifikatoren er **deterministisk**, ikke et språkmodellkall, og dekker tre av punktene over: **riktige numeriske verdier**, **riktig gjengivelse fra kilden** og **at agenten ikke har fylt inn ikke-rapporterte data** — det siste i den ene retningen en maskin kan avgjøre det, som et kildeomfattende søk etter en verdi raden fører som fraværende (§19.1). Den henter kildeversjonens adresse på nytt, sammenligner fingeravtrykket, og søker deretter hver ordrett gjengivelse i `raw_extraction` og hvert oppgitt tall i den hentede representasjonen. `ANTIDEP_CONSTITUTION.md` §17 ber om determinisme «der det er mulig», og for sitat- og tallkontroll er det både mulig og strengere enn en modellvurdering.
