...
**Et kontrollfelt skal aldri kunne bære en påstand operasjonen ikke gjorde.** `not_reported` og `not_measured` gjelder kildeversjonen som helhet, og en menneskelig kontrolløkt ser ett lokalt utdrag. Påstanden er derfor delt i to felter i `workflow.evidence_check_field`: `availability_semantics` er den lokale halvdelen, som et menneske avgjør av utdraget, og `source_wide_absence` er den kildeomfattende, som krever at to reproduserbare maskinelle ledd er enige om hele den registrerte representasjonen: et deterministisk søk som kan avkrefte fraværet, og en uavhengig gjennomlesning som kan konkludere. Et negativt søkeresultat dekker feltet aldri alene — naturlig språk har ingen uttømmende mønsterliste, og et fravær kan ikke bevises av et søk (issue #74). `workflow.required_check_fields(uuid)` krever den andre når og bare når raden fører et slikt fravær (`workflow.source_wide_absence_fields(uuid)`), og `evidence_verifications_source_wide_absence_check` gjør det umulig — ikke bare frarådet — for en kontroll uten agentkjøring å føre den opp. Uten den delingen ville et «ja» på det lokale spørsmålet blitt bokført som et svar på det globale (EVIDENCE_PIPELINE.md §19.1, §25.1).

Hvilken av flere registrerte kontroller som er *den gjeldende*, avgjøres av et databaseeid registreringsnummer (`registration_ordinal`) og ikke av tidsstemplene. Nummeret tildeles fra en sekvens av en trigger, på innsiden av den radlåsen skriveveien allerede tar, og følger derfor nødvendigvis den rekkefølgen radene faktisk ble skrevet i. Tidsstemplene kan ikke brukes til det: `now()` er transaksjonens starttidspunkt, så to samtidige registreringer kan starte i én rekkefølge og skrive i den motsatte — og et avvik som ble skrevet sist ville da kunnet bære det eldste tidsstempelet og bli sortert bak en bekreftelse som ble skrevet før det. Publiseringsgaten, dekningen (`workflow.covered_check_fields`), maskinbeviset (`workflow.grounding_machine_proved`) og reviewerflatene leser alle det samme nummeret, slik at de ikke kan bli uenige om hvilken rad som er den siste. Tidsstemplene beholdes og leses fortsatt der spørsmålet er *når* noe ble gjort — mandatkontrollene måler rollen mot radens eget `verified_at`. Den samme regelen gjelder `workflow.claim_verifications` (§30).
