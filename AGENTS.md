...
Årsaken legges i en utboks i nettleseren _før_ den sendes, og blir liggende til leveringen er bekreftet (`src/app/diagnostics-outbox.ts`). Er lagringen utilgjengelig, kommer årsakene fram ved neste sending eller neste gang appen åpnes — utsatt framfor mistet. Hver observasjon har et nummer, så et nytt forsøk blir den samme raden og ikke en til. En uinnlogget besøkende sender ingenting: en rad som ikke kan tilskrives noen, ville vært en åpen skrivevei. Observasjonen blir liggende, og den første innloggingen i den nettleseren tar den med. Transportformen bærer mest når koden mangler, altså nettopp når svaret aldri kom. En selvmeldt rad er et hjerteslag og ikke en tilstand: den gjelder så lenge den fornyes, og databasen lukker den selv. Flaten lukker aldri noe — en opprydding som hvilte på klientens minne, ville vært borte ved første sideoppfriskning.
