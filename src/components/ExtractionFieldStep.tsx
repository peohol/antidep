...
export function ExtractionFieldStep({
  ...
  /**
   * Bærer teksten fortsatt papirets plassering? Da vises utdraget som det står,
   * framfor å flyte — se `excerptKeepsLayout` i `../lib/readable-excerpt.ts`.
   */
  readonly keepsLayout: boolean
      ...
      <div className="field-check__panes">
        <div className="field-check__pane">
          ...
          {/* Utdraget som lesbar tekst: ett avsnitt per uavhengig tekstblokk,
              ...
              Med ett unntak: bærer teksten fortsatt papirets plassering, vises
              den som den står. Da ligger to spalter på den samme tekstlinjen, og
              mellomrommet mellom dem er det eneste synlige varselet om at
              ordene ikke hører sammen. */}
          {keepsLayout ? (
            <>
              <p className="field-check__excerpt-warning" role="note">
                Denne teksten er hentet ut uten Antideps rekonstruksjon av leserekkefølgen, og står
                derfor med plasseringen fra papiret. Ord som er langt fra hverandre på en linje, kan
                tilhøre to forskjellige spalter og ikke samme setning.
              </p>
              <blockquote className="field-check__excerpt field-check__excerpt--as-placed">
